...
def load_data(dataset_path, tokenizer, max_model_len, max_tokens):
    ...
    for split in ['train', 'test']:
        for instance in data[split]:
            ...
            input_len = len(tokenizer(instance['prompt'], add_special_tokens=False)['input_ids'])
...
def compute_score(completions, labels, score_sets, reward_1, max_tokens, tokenizer, **kwargs):
    """
    ...
    The reward is based on:
      ...
      Completions are lists of strings for tts experiments
    """
    ...
    def get_reward(s, label, previous_reward, score_set, length):
        try:
            score = float(s.strip())
        except (ValueError, TypeError):
            return 0
        if score in score_set:
            if (score == label) and previous_reward == 1.5:
                raw_score = 0.5
            elif (score == label) and previous_reward == 0.5:
                raw_score = 1
            elif (score != label) and previous_reward == 1.5:
                raw_score = -1
            else:
                raw_score = -0.5
            return raw_score - length_penalty(length)
        else:
            return 0
    ...
    rewards = np.array([get_reward(m.group(1), label, previous_reward, score_set, length) if m else -0.5 for m, label, previous_reward, score_set, length in zip(matches, labels, reward_1, score_sets, lengths)])

    print(f"\nEXAMPLE FROM BATCH\n\nCompletion: {completions[0]}\n\nLabel: {labels[0]}\n\nPrevious Reward: {reward_1[0]}\n\nReward: {rewards[0]}\n\n")
